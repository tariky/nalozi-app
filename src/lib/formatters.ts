...
// Get status color class - Lyra outline badge style
export function getStatusColor(status: string): string {
  const colors: Record<string, string> = {
    otvoren: 'bg-transparent text-status-info border-status-info',
    u_toku: 'bg-transparent text-status-warning border-status-warning',
    zavrsen: 'bg-transparent text-status-success border-status-success',
  };
  return colors[status] || 'bg-transparent text-muted-foreground border-border';
}
